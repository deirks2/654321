...
# ─── 상태 저장 ───────────────────────────────────────
# Gemini 히스토리 형식: {"role": "user"/"model", "parts": ["..."]}
...
async def ask_ai(channel_id: int, user_name: str, prompt: str) -> str:
    ...
    # 기존 대화 기록 기반으로 채팅 세션 생성
    chat = gemini_model.start_chat(history=history)

    # 메시지 전송
    response = chat.send_message(f"[{user_name}]: {prompt}")
    reply = response.text

    # 히스토리 업데이트
    conversation_history[channel_id].append(
        {"role": "user", "parts": [f"[{user_name}]: {prompt}"]}
    )
    conversation_history[channel_id].append(
...
async def remove_channel(ctx, channel: discord.TextChannel = None):
    ...
    if not allowed_channels:
        status = "허용 채널이 없어서 다시 **모든 채널**에서 동작해요."
    else:
        status = f"남은 허용 채널: {len(allowed_channels)}개"
...
async def clear_history(ctx):
    ...
    await ctx.send("🔄 이 채널의 대화 기록을 초기화했어요! 새 대화를 시작할게요.")
